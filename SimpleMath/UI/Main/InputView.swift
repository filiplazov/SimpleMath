...
struct InputView: View {
  ...
  var body: some View {
    HStack {
      CommandInputButton(symbol: .erase, color: .discard, action: viewModel.erase, isEnabled: viewModel.commandsAvailable)
      ...
      CommandInputButton(symbol: .checkmark, color: .confirm, action: viewModel.evaluate, isEnabled: viewModel.commandsAvailable)
        .font(.commandFont(for: maxWidth, horizontalSizeClass: hSizeClass))
      .padding(.horizontal, hSizeClass.isRegular ? 60 : 0)
    }
  }
